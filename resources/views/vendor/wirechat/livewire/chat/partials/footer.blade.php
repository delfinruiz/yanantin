...
<footer class="shrink-0 h-auto relative   sticky bottom-0 mt-auto">
    ...
    @else
        <div id="chat-footer" x-data="{ 'openEmojiPicker': false }"
            ...
            @php
                $wirechatConfigData = [
                    'maxFiles' => $this->panel()->getMaxUploads(),
                    'config' => [
                        'media' => [
                            'maxSize' => $this->panel()->getMediaMaxUploadSize() * 1024,
                            'mimes' => $this->panel()->getMediaMimes(),
                            'validation' => [
                                'max' => __('wirechat::validation.max.file', ['attribute' => __('wirechat::chat.inputs.media.label'),'max'=>$this->panel()->getMediaMaxUploadSize()]),
                                'mimes' => __('wirechat::validation.mimes', [ 'attribute' => __('wirechat::chat.inputs.media.label'), 'values' => implode(', ', $this->panel()->getMediaMimes()) ])
                            ]
                        ],
                        'files' => [
                            'maxSize' => $this->panel()->getFileMaxUploadSize() * 1024,
                            'mimes' => $this->panel()->getFileMimes(),
                            'validation' => [
                                'max' => __('wirechat::validation.max.file', ['attribute' => __('wirechat::chat.actions.upload_file.label'),'max'=>$this->panel()->getFileMaxUploadSize()]),
                                'mimes' => __('wirechat::validation.mimes', [ 'attribute' => __('wirechat::chat.actions.upload_file.label'), 'values' => implode(', ', $this->panel()->getFileMimes()) ])
                            ]
                        ]
                    ],
                    'messages' => [
                        'maxFiles' => __('wirechat::validation.max.array', ['attribute' => __('wirechat::chat.inputs.media.label'),'max'=>$this->panel()->getMaxUploads()])
                    ]
                ];
            @endphp
            <div id="wirechat-config-data" data-config="{{ json_encode($wirechatConfigData) }}" style="display:none;"></div>

            @script
                <script>
                    const wirechatConfig = JSON.parse(document.getElementById('wirechat-config-data').dataset.config);
                    ...
                    Alpine.data('attachments', (type = "media") => ({
                        ...
                        MAXFILES: wirechatConfig.maxFiles, // Maximum number of files allowed
                        progress: 0, // Progress of the current upload (0-100)
                        wireModel: type, // The Livewire model to bind to
                        
                        config: wirechatConfig.config,

                        get maxSize() {
                            return this.config[this.type].maxSize;
                        },

                        get allowedFileTypes() {
                            return this.config[this.type].mimes;
                        },
                        ...
                        validateFiles(files, count) {
                            ...
                            if (totalFiles > this.MAXFILES) {
                                ...
                                $dispatch('wirechat-toast', {
                                    type: 'warning',
                                    message: wirechatConfig.messages.maxFiles
                                });
                            }
                            ...
                            const invalidFiles = Array.from(files).filter((file) => {
                                ...
                                return file.size > this.maxSize || !this.allowedFileTypes.includes(extension); // Check size and type
                            });

                            // Filter valid files
                            const validFiles = Array.from(files).filter((file) => {
                                const extension = file.name.split('.').pop().toLowerCase();
                                return file.size <= this.maxSize && this.allowedFileTypes.includes(extension);
                            });
                            ...
                            if (invalidFiles.length > 0) {
                                invalidFiles.forEach((file) => {
                                    if (file.size > this.maxSize) {
                                        $dispatch('wirechat-toast', {
                                            type: 'warning',
                                            message: this.config[this.type].validation.max
                                        });
                                    } else {
                                        const extension = file.name.split('.').pop().toLowerCase();
                                        $dispatch('wirechat-toast', {
                                            type: 'warning',
                                            message: this.config[this.type].validation.mimes
                                        });
