<div
    ...
    x-data="{
        ...
        url: '{{ \App\Filament\Resources\Meetings\MeetingResource::getUrl() }}',
        init() {
            this.updateDom();
        ...
        updateDom() {
            ...
            if (!link) {
                ...
                link = items.find(el => el.textContent?.trim() === '{{ \App\Filament\Resources\Meetings\MeetingResource::getNavigationLabel() }}');
            }
            if (!link) return
            let badge = link.querySelector('#fp-meetings-badge') || link.querySelector('.fi-sidebar-item-badge') || link.querySelector('.fi-badge')
