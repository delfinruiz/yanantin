<div
    ...
    x-data="{
        ...
        url: '{{ \App\Filament\Resources\Tasks\TaskResource::getUrl() }}',
        init() {
            this.updateDom();
        ...
        updateDom() {
            ...
            if (!link) {
                ...
                link = items.find(el => el.textContent?.trim() === '{{ \App\Filament\Resources\Tasks\TaskResource::getNavigationLabel() }}');
            }
            if (!link) return
            let badge = link.querySelector('#fp-tasks-badge') || link.querySelector('.fi-sidebar-item-badge') || link.querySelector('.fi-badge')
